...
import { Sparkles, Clock, Tag, MoreVertical, Plus, Edit, Power, PowerOff } from "lucide-react"
      ...
      <div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {initialServices.map((s: any) => (
            <Card key={s.id} className={`hover:shadow-md transition-shadow ${!s.is_active ? "opacity-75" : ""}`}>
              <CardContent className="p-5 space-y-3">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-2">
                    <div className={`h-9 w-9 rounded-lg flex items-center justify-center ${s.is_active ? "bg-indigo-100 dark:bg-indigo-900/50 text-indigo-600" : "bg-slate-100 text-slate-500"}`}>
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="secondary" className={s.is_active ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}>
                ...
                <p className="text-sm text-muted-foreground line-clamp-2">{s.short_description}</p>
                <div className="flex items-center justify-between pt-2 border-t text-sm">
                  <div className="flex items-center gap-1 text-muted-foreground">
                    <Clock className="h-3.5 w-3.5" />
                    {Math.floor(s.duration_minutes / 60)}h {s.duration_minutes % 60 > 0 ? `${s.duration_minutes % 60}m` : ""}
                  </div>
                  <span className="font-bold text-indigo-600">£{Number(s.base_price).toFixed(0)}</span>
                </div>
              </CardContent>
            </Card>
          ))}
